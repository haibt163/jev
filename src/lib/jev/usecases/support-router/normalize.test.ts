...
import { deriveWorkflow } from "./rules.ts";
...
describe("missing fields", () => {
  ...
  it("rejects a missing intent_category", () => {
    const { intent_category: _omitted, ...rest } = validAnswers();
...
describe("score boundaries", () => {
  it("accepts scores at 0 and max", () => {
    const atZero = composeProfile(validAnswers({ urgency: score(0) }));
    const atMax = composeProfile(validAnswers({ urgency: score(4) }));
    assert.equal(atZero.ok, true);
    assert.equal(atMax.ok, true);
  });

  it("rejects a score far outside the rubric", () => {
    const result = composeProfile(validAnswers({ urgency: score(9) }));
    assert.equal(result.ok, false);
  });
});
...
describe("application workflow rules", () => {
  it("holds ambiguous requests for clarification before routing", () => {
    const result = composeProfile(validAnswers({ ambiguity: noul(0.9) }));
    assert.equal(result.ok, true);
    if (!result.ok) return;
    const workflow = deriveWorkflow(result.profile);
    assert.equal(workflow.action, "clarify");
    assert.match(workflow.explanation, /ambiguous/);
  });

  it("flags review when escalation noul crosses the threshold", () => {
    const result = composeProfile(validAnswers());
    assert.equal(result.ok, true);
    if (!result.ok) return;
    const workflow = deriveWorkflow(result.profile);
    assert.equal(workflow.action, "route_and_flag");
    assert.match(workflow.explanation, /person/);
  });

  it("routes automatically when nothing is flagged", () => {
    const result = composeProfile(
      validAnswers({
        human_escalation: noul(0.1),
        urgency: score(1),
      }),
    );
    assert.equal(result.ok, true);
    if (!result.ok) return;
    const workflow = deriveWorkflow(result.profile);
    assert.equal(workflow.action, "route");
    assert.match(workflow.explanation, /automatically/);
  });
});
